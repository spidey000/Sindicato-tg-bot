...
class TestProgressUpdates(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_progress_status_transitions(self, mock_gen_id, mock_docs, mock_drive, mock_notion, mock_orch, mock_update_progress, mock_send_progress):
        ...
        mock_agent.generate_structured_draft.return_value = {"summary": "S", "content": "C"}
        ...
        # We expect 8 calls (1 initial pending + 7 completions)
        self.assertEqual(mock_update_progress.call_count, 8)
        
        # Verify the content of some calls to ensure status changes from pending to completed
        # Call 1: Drafting -> pending (redundant call in handler)
        ...
        # Call 3: Initialization -> completed
        args3 = mock_update_progress.call_args_list[2][0]
        steps_status3 = args3[3]
        self.assertEqual(steps_status3[1], ["Initialization", "completed"])
        
        # Call 8: Docs Creation -> completed
        args8 = mock_update_progress.call_args_list[7][0]
        steps_status8 = args8[3]
        self.assertEqual(steps_status8[6], ["Docs Creation", "completed"])
