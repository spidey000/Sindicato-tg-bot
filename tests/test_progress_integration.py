...
class TestProgressIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("src.handlers.docs")
    @patch("src.handlers.generate_case_id")
    async def test_email_handler_progress_flow(self, mock_gen_id, mock_docs, mock_drive, mock_notion, mock_orch, mock_update_progress, mock_send_progress):
        # Setup mocks
        ...
        context = MagicMock()
        context.args = ["Solicitud", "Vacaciones"]
